...
assert.match(installer, /MENU_SELECT_OUT/)
assert.ok(!/choice=\$\(node "\$SCRIPT_DIR\/lib\/menu-select\.mjs"/.test(installer))
assert.match(initSh, /MENU_SELECT_OUT/)
assert.match(dispatch, /function markCliInteractive/)
assert.match(dispatch, /function runPlatformPm/)
assert.match(dispatch, /runPlatformPm\(platform\)/)
assert.match(dispatch, /function runPlatformGitSetup/)
assert.match(dispatch, /runPlatformGitSetup\(\)/)
assert.match(dispatch, /process\.stdin\.isTTY/)
assert.ok(!/process\.stdin\.isTTY \|\| process\.stdout\.isTTY/.test(dispatch))
assert.match(source, /MENU_SELECT_OUT/)
assert.ok(!/choice=\$\(node "\$menu_js"/.test(source))
assert.match(installer, /lib\/string-width\.mjs/)
...
assert.match(source, /menu\.mjs already printed Canceled/)
...
assert.ok(syncLocals.some((local) => String(local).includes('.config/homebrew/lib/string-width.mjs')))
assert.ok(syncLocals.some((local) => String(local).includes('.config/homebrew/lib/tty-term.mjs')))
assert.ok(syncRepos.some((repo) => String(repo).includes('scripts/lib/string-width.mjs')))
