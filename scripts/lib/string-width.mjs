...
export function resolveAmbiguousWide(env = process.env, intlLocale) {
  ...
  for (const raw of locales) {
    ...
    if (locale === 'C' || locale === 'POSIX') continue
    if (/^(zh|ja|ko)([-_]|$)/i.test(locale)) return true
    if (/[-_](CN|Hans|Hant|JP|KR|TW|HK)\b/i.test(locale)) return true
  }
  return false
}
...
export function codePointWidth(codePoint, ambiguousWide = false) {
  ...
  if (codePoint <= 0x7e) return 1
  if (isWideCodePoint(codePoint)) return 2
  if (isAmbiguousCodePoint(codePoint)) return ambiguousWide ? 2 : 1
  return 1
}
...
export function truncateWidth(text, max, options = {}) {
  const value = String(text ?? '')
  if (max <= 0) return ''
  if (stringWidth(value, options) <= max) return value
