...
assert.match(scoopZsh, /_scoop_ps_quote/)
assert.match(scoopZsh, /-Command/)
...
assert.match(servicesHelper, /Invoke-ScoopServicesManager -Tokens/)
// Do NOT use ValueFromRemainingArguments with -File (empties both CommandArgs and $args).
assert.ok(!/\[Parameter\([^\]]*ValueFromRemainingArguments/.test(servicesHelper))
...
assert.match(scoopZsh, /cygpath/)
assert.match(scoopZsh, /\[\[ "\$a" == -\* \]\]/)
...
assert.match(bootstrap, /Installing Scoop via/)
assert.match(bootstrap, /Scoop installed/)
...
assert.match(bootstrap, /Invoke-Expression \$expression\s*\|\s*Out-Null/,
  'installer Write-Output must be discarded, not joined into ActivePrefix')
assert.match(bootstrap, /Invoke-QuietHost/,
  'installer host chatter must be suppressed like mac install.sh spin')
...
assert.ok(!/ForEach-Object\s*\{\s*Write-Host/.test(bootstrap), 'do not bare-print installer Write-Output')
...
const restorePm = read('src/pm/restore.js')
assert.match(initTs, /restoreScoopPackages/)
assert.match(restorePm, /runtime\/scoop\/import-backup\.ps1/)
...
assert.match(utilsPs, /function Invoke-QuietHost/, 'one-click flows must be able to hide Scoop Write-Host noise')
...
assert.match(rootInstall, /function Get-UrlHostLabel/, 'windows install should label mirrors by host like install.sh')
assert.match(rootInstall, /Syncing \$hostLabel/)
assert.ok(!/Trying sync URL:/.test(rootInstall), 'do not bare-print full sync URLs')
assert.ok(!/Trying zip URL:/.test(rootInstall), 'do not bare-print full zip URLs')
assert.ok(!/Trying clone URL:/.test(rootInstall), 'do not bare-print full clone URLs')
