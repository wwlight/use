...
_brew_mirror_ensure_profile() {
...

    {
        ...
        # Portable refs — do not embed absolute $HOME paths (usernames) in .zprofile.
        printf '%s\n' '[[ -r "${XDG_CONFIG_HOME:-$HOME/.config}/homebrew/mirror.zsh" ]] && . "${XDG_CONFIG_HOME:-$HOME/.config}/homebrew/mirror.zsh"'
        printf '%s\n' '[[ -r "${XDG_CONFIG_HOME:-$HOME/.config}/homebrew/mirror-cli.zsh" ]] && . "${XDG_CONFIG_HOME:-$HOME/.config}/homebrew/mirror-cli.zsh"'
        printf '%s\n' "$end"
    } >> "$tmp"
    mv "$tmp" "$profile"
}
...
_brew_mirror_menu_cli() {
    local candidate dir=""
    local -a candidates=()
    candidates+=("$(_brew_mirror_root)/lib/mirror-menu.js")
    ...
    [[ -n "$dir" ]] && candidates+=("$dir/mirror-menu.js" "$dir/lib/mirror-menu.js")
...
# Interactive ↑↓ select via mirror-menu.js → menu-select.
_brew_mirror_select_interactive() {
    ...
    menu_js=$(_brew_mirror_menu_cli) || {
        printf 'brew mirror: mirror-menu.js not found\n' >&2
        return 1
    }
...
_brew_mirror_cli() {
    ...
    if [[ -z "$choice" ]]; then
        if ! choice=$(_brew_mirror_select_interactive); then
            local ec=$?
            # mirror-menu.js already printed Canceled on 130
            return "$ec"
        fi
        local active
        active="${USE_HOMEBREW_MIRROR:-}"
