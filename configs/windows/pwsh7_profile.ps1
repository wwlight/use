...
function _scoop_ensure_xml {
  ...
  $stopExe = $cfg.stopexecutable ? "%BASE%/../../apps/$name/current/$($cfg.stopexecutable)" : $exe
  $argsEl = $cfg.arguments ? "`n  <arguments>$($cfg.arguments)</arguments>" : ""
  $stopArgsEl = $cfg.stoparguments ? "`n  <stoparguments>$($cfg.stoparguments)</stoparguments>" : ""
