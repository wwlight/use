...
# Aliases (shared with pwsh 5) — ~/.config/pwsh/aliases.ps1
$__pwshCfg = if ($env:XDG_CONFIG_HOME) { Join-Path $env:XDG_CONFIG_HOME 'pwsh' } else { Join-Path $env:USERPROFILE '.config\pwsh' }
$__pwshAliases = Join-Path $__pwshCfg 'aliases.ps1'
if (Test-Path -LiteralPath $__pwshAliases) { . $__pwshAliases }
